...
EXPERIMENT="LinearRoadAccident"

echo "Starting experiment. Results will be stored in data/output/$EXPERIMENT_FOLDER"
sleep 2

./scripts/run.py ./experiments/predicate/"$EXPERIMENT".yaml -d "$DURATION" -r "$REPS" --kafkaHost "$KAFKA_HOST" -c "$DATE_CODE"

./reproduce/plot.py --path "data/output/$EXPERIMENT_FOLDER" --plot averages