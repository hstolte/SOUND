...
./scripts/run.py ./experiments/overhead/SmartGridAnomalyOverheadByMaxSamples.yaml -d "$DURATION" -r "$REPS" -c "$DATE_CODE"

cmd="./reproduce/plot.py --path \"data/output/$EXPERIMENT_FOLDER\" --plot overhead_nsamples"

echo "To re-run only the plot, execute: $cmd"
bash -c "$cmd"

#./reproduce/plot.py --path "data/output/$EXPERIMENT_FOLDER" --plot time-series
