package io.stolther.soundcheck.usecases.smartgrid.sound;
...
public class SmartGridAnomalyCaseWithUncertainty {
    ...
    public static void main(String[] args) throws Exception {
        ...
        UnarySequenceCheck SGF_3 = new UnarySequenceCheck() {
            @Override
        ...
        UnaryPointBasedCheck SGF_2 = new UnaryPointBasedCheck() {
            @Override
            protected boolean constraint(double x) {
                return x > 10; // todo find plausible range
            }
        };
        SGF_2.init(settings);
        ...
        BinarySetCheck SGF_8 = new BinarySetCheck() {
            @Override
        ...
        UnarySetCheck SGF_7 = new UnarySetCheck() {
            @Override
            public boolean constraint(double[] x) {
        ...
        // todo update the following block everywhere else
        final DataStream<AnomalyResultTuple> anomalyResultStream = householdUsageStream
        ...
        final DataStream<AnomalyResultTuple> outputStream = anomalyResultStream
        ...
        UnaryPointBasedCheck SGF_6 = new UnaryPointBasedCheck() {
            @Override
            protected boolean constraint(double x) {
        ...
        anomalyResultStream.map((MapFunction<AnomalyResultTuple, DataPoint>) tuple ->
                        new DataPoint(
                                tuple.timestamp, // todo update in other queries too ?
                                tuple.difference,
                                settings.manual_value_uncertainty,
                                tuple.houseId + "/" + tuple.householdId))
                .map(SGF_6)
                .addSink(new CheckResultSink("SGF-6", settings))
                .setParallelism(settings.sinkParallelism());
