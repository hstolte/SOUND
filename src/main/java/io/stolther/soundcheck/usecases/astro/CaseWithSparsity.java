...
//        return new NoOpCheckResultSink();
    ...
    public static void main(String[] args) throws Exception {
        ...
        APP_1.init(settings);
        filtered
                .map((MapFunction<AstroDataPoint, DataPoint>) t -> new DataPoint(t.time, t.flux, t.sigma, t.sourceName))
                .map(APP_1)
...
//                })
        ...
        twoSeriesStream.map(A3).addSink(get_check_sink("A-3", settings));
        ...
        A4.init(settings);
        twoSeriesStream.map(A4).addSink(get_check_sink("A-4", settings));
